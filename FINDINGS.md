...
### Causal-importance pipelines must use magnitude-INDEPENDENT injection, or they collapse to a magnitude ranking

If you score an SAE feature's causal importance by perturbing its direction and measuring downstream change, the perturbation magnitude must be the *same for every feature*. The intuitive choice — scale the injection by the feature's own activation (so the perturbation is "in units of how active it usually is") — is correct for an interactive *intervention* tool but fatal for an *importance ranking*: bigger perturbations cause bigger downstream changes, so the score becomes a re-expression of activation. Measured here: with magnitude-relative injection, `corr(causal_score, activation) = +0.37` and the top causal features were exactly the top-activation features in order; switching to fixed-norm injection (`scale × unit_direction`) dropped that to ≈0 and produced a genuinely different ranking.

The trap is that this is **invisible in the per-feature math** — each feature's score looks like a clean causal measurement — and only shows up at the **population-level cross-check** (correlate the score against activation across all features). Always run that cross-check before trusting a causal ranking.

### A deterministic env + a confident policy means "more seeds" doesn't diversify the seed state — and a causal score from one state isn't a ranking

Causal importance was measured by intervening during an imagined rollout from a "seed state" reached by warming up the real env. Two surprises: (1) the env reset and the trained (near-argmax) policy are **effectively deterministic**, so re-running — and even re-seeding the RNG — produces the *same* seed state (verified: bit-identical scores across runs and across torch seeds). The causal score therefore reflected essentially **one game state**. (2) Removing the magnitude confound *exposed* this — the confound had been masking it. Genuine diversity had to come from sampling different *points* along a playthrough, not from re-seeding.

Even with diverse states, cross-set robustness (two disjoint state-sets) only climbed 1→5→18 states as ~0.0 → +0.33 → +0.49 Spearman — improving but with sharp diminishing returns, never clearing a usable bar. **Implication:** a causal/intervention-effect ranking on a world model is single-state-fragile; it needs averaging over many genuinely-diverse states to generalize, and "increase the seed count" does nothing if your states aren't actually diverse. Verify state diversity explicitly (do two disjoint state-sets agree?), and treat a single-state causal ranking as a per-state probe, not a feature ranking.

### Autoregressive rollout pipelines are dispatch-bound, not GPU-bound — a bigger GPU doesn't help

The causal pipeline rolls the world model forward one token at a time in a Python loop (autoregressive, KV-cached). Each call is a tiny forward the GPU finishes in microseconds, then sits idle waiting for Python to dispatch the next. At scale (18 states × 80 features × 2 signs × 10 steps × 17 passes ≈ 490k serial calls per run) the cost is **per-call dispatch overhead**, confirmed by CPU-time ≈ wall-time and ~100% CPU with the GPU near-idle. Moving from an M1 to an A100 barely changed the per-call rate, and running two such jobs in parallel *contended* for CPU rather than parallelizing (it's CPU-bound, not GPU-bound). **Implication:** to actually use a GPU for this, batch the rollouts (all features/states/signs as batched forwards) — turning ~1M serial calls into a few thousand batched ones. Until then, throwing bigger/more GPUs at an autoregressive eval loop is wasted money; the bottleneck is Python dispatch.
